...
/**
 * $Id: indexpage.php v 1.0.3 05 july 2004 Liquid Exp $
 * Module: PD-Links
 * Version: v1.0
...
switch ($op) {
    case 'save':
        ...
        $xoopsDB->query(
            'update '
            . $xoopsDB->prefix('PDlinks_indexpage')
        ...
        redirect_header('indexpage.php', 1, _AM_PDD2_IPAGE_UPDATED);
        exit();
    ...
    default:
        ...
        $result = $xoopsDB->query('SELECT indeximage, indexheading, indexheader, indexfooter, nohtml, nosmiley, noxcodes, noimages, nobreak, indexheaderalign, indexfooteralign FROM ' . $xoopsDB->prefix('PDlinks_indexpage') . ' ');
        [$indeximage, $indexheading, $indexheader, $indexfooter, $nohtml, $nosmiley, $noxcodes, $noimages, $nobreak, $indexheaderalign, $indexfooteralign] = $xoopsDB->fetchrow($result);

        xoops_cp_header();
        PDd_adminmenu(_AM_PDD2_INDEXPAGE);

        echo "
		<fieldset><legend style='font-weight: bold; color: #900;'>" . _AM_PDD2_IPAGE_INFORMATION . "</legend>\n
		<div style='padding: 8px;'>" . _AM_PDD2_MINDEX_PAGEINFOTXT . "</div>\n
		</fieldset>\n
		";

        $sform = new XoopsThemeForm(_AM_PDD2_IPAGE_MODIFY, 'op', xoops_getenv('PHP_SELF'));
        $sform->addElement(new XoopsFormText(_AM_PDD2_IPAGE_CTITLE, 'indexheading', 60, 60, $indexheading), false);
        ...
        $indeximage_tray = new XoopsFormElementTray(_AM_PDD2_IPAGE_CIMAGE, '&nbsp;');
        ...
        $sform->addElement(new XoopsFormDhtmlTextArea(_AM_PDD2_IPAGE_CHEADING, 'indexheader', $indexheader, 15, 60));
        $headeralign_select = new XoopsFormSelect(_AM_PDD2_IPAGE_CHEADINGA, 'indexheaderalign', $indexheaderalign);
        $headeralign_select->addOptionArray(['left' => _AM_PDD2_IPAGE_CLEFT, 'right' => _AM_PDD2_IPAGE_CRIGHT, 'center' => _AM_PDD2_IPAGE_CCENTER]);
        $sform->addElement($headeralign_select);
        $sform->addElement(new XoopsFormTextArea(_AM_PDD2_IPAGE_CFOOTER, 'indexfooter', $indexfooter, 10, 60));
        $footeralign_select = new XoopsFormSelect(_AM_PDD2_IPAGE_CFOOTERA, 'indexfooteralign', $indexfooteralign);
        $footeralign_select->addOptionArray(['left' => _AM_PDD2_IPAGE_CLEFT, 'right' => _AM_PDD2_IPAGE_CRIGHT, 'center' => _AM_PDD2_IPAGE_CCENTER]);
        ...
        $button_tray->addElement(new XoopsFormButton('', 'post', _AM_PDD2_BSAVE, 'submit'));
